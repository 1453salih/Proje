...
def calculate_similarity_endpoint():
    try:
        ...
        print(f"İlan Metni: {job_text}")  # Temizlenmiş ilan metnini ekrana yazdırır

        similarity_score = calculate_similarity(resume_text, job_text)
        similarity_score = round(similarity_score, 2)

        # Benzerlik puanını eşik değerlerine göre ayarlayalım
        if similarity_score <= 85:
            similarity_score -= 40
        elif 85 < similarity_score <= 90:
            similarity_score -= 20
        elif 90 < similarity_score <= 99:
            similarity_score -= 10
